...
import { SEND_EVENT, RECEIVE_EVENT, CONNECTION_EVENT, DISCONNECTION_EVENT } from "spinal-service-pubsub-logs";
...
export class SocketHandler {
	...
	public listenSubscribeEvent(socket: Socket) {
		socket.on(SUBSCRIBE_EVENT, async (...args) => {
			...
			await this._createLog(socket, RECEIVE_EVENT, `${RECEIVE_EVENT}_${SUBSCRIBE_EVENT}_event`);
		});
	}
	...
	public listenUnsubscribeEvent(socket: Socket) {
		socket.on(UNSUBSCRIBE_EVENT, async (...args) => {
			...
			// log
			await this._createLog(socket, RECEIVE_EVENT, `${RECEIVE_EVENT}_${UNSUBSCRIBED}_event`);
		});
	}
	...
	public async sendSocketEvent(node: SpinalNode, model: { info: { [key: string]: any }; element: { [key: string]: any } }, eventName: string, action?: IAction, socket?: Socket) {
		...
		for (const _socket of sockets || []) {
			...
			// log
			await this._createLog(_socket, SEND_EVENT, `${SEND_EVENT}_${event}_event`, data.node);
		}
	}
	...
	private _bindNodes(socket: Socket, result: IGetNodeRes[], nodes: { [key: string]: INodeData }, sessionId: string): Promise<INodeId[]> {
		return result.reduce(async (prom, { error, nodeId, status, eventNames, options }) => {
			...
			if (!error && status === OK_STATUS) {
				...
				await spinalGraphUtils.recursionFunction(recursionArg, spinalGraphUtils.bindNode.bind(spinalGraphUtils));
	...
	private async _leaveRoom(socket: Socket, result: IGetNodeRes[], nodes: { [key: string]: INodeData }): Promise<INodeId[]> {
		return result.reduce(async (prom, { error, nodeId, status, eventNames, options }) => {
			...
			if (!error && status === OK_STATUS) {
				...
				await spinalGraphUtils.recursionFunction(recursionArg, (arg: IRecursionArg) => {
	...
	public _createLog(socket: any, type: string, action: string, nodeInfo?: { id: string; name: string; [key: string]: string }) {
